...
| `tree_utils.py` | Split | Move analysis/config LDict helpers to `feedbax.config.tree` or `feedbax.config.tree`. Consider upstreaming reusable LDict level rearrangement helpers to `jax_cookbook` only after tests. Use JAX core for simple path flattening. |
