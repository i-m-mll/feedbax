...
`feedbax.orchestration.run_set_state.v6` — because that string is what is written
...
The versioned policy manifest is the structured authority for every row's
public imports, command names, schema transitions, external cases, and boundary
obligations. The table below is its human-readable rendering. A row guarantees
only the names in its `public_api` block; a used name does not silently promote
its neighboring helpers or re-export locations into the contract.
...
| `component-registration` | `feedbax.component_registry` | `ComponentBuilder`, `DeclaredComponent`, `declare_component`, `ComponentResolution`, `ComponentRegistry`, `ComponentMigration`, `ComponentMigrationPack` | Neutral component declarations with layer-local facets, namespaced registration, owner-matched migration packs, deterministic resolution, and the stable methods `ComponentRegistry.register_component_type`, `register_migration`, `register_migration_pack`, and `resolve_component_spec`. `feedbax.spec.component_definition.v3` is current; v1 migrates to v2 port kinds and v2 migrates to v3 dynamic-port policy. Dynamic layouts derive only from declared policy and parameters and reject mismatches. `ComponentDefinition` remains Studio data, not a separately guaranteed Python import. | `component_registration_and_migration`, `dynamic_component_ports` |
...
| `material-admission` | `feedbax.contracts.material_dependencies` and the same names at `feedbax.contracts` | `AdmissionWaiver`, `MaterialDependency`, `MaterialDependencySet`, `MaterialDependencyValue`, `validate_material_dependency_admission` | Material dependencies use `feedbax.spec.material_dependencies.v1`; the exact narrow waiver uses `feedbax.spec.admission_waiver.v1`. Versionless and other versions reject. Material identity excludes incidental provenance and declaration order; a waiver binds exactly one declared dependency and never bypasses material authenticity. Observation, result, and digest helpers are implementation details rather than independent guarantees. | `material_dependencies` |
| `custody-persistence` | `feedbax.orchestration` | `PrimaryStatePersistenceError`, `CustodyPreservationRequired`, `RunSetState`, `RunSetStateStore`, `StageEngine` | `feedbax.orchestration.run_set_state.v6` is current; v5 migrates by adding nullable process identity to each row, and versions v0 through v4 reject. Primary persistence failure retains its typed `OSError` cause; destructive ephemeral teardown is blocked by the typed custody exception until collection is durably complete, then occurs exactly once. | `custody_persistence_recovery` |
| `emergency-persistence` | `feedbax.orchestration` | `ControlFilesystemPreflightError`, `EmergencyRunSetRecord`, `RunSetStateStore` | `feedbax.orchestration.emergency_run_set_record.v1` is current; v0, unknown, and malformed records reject. The bounded reserved channel publishes and reads back provider identity, preservation state, custody completion, spend boundary, primary failure, and next recovery action before destructive teardown. Nested provider identity and successful-preflight result shapes remain durable data, not separate import promises. | `custody_persistence_recovery` |
...
## Structural and command obligations

Some downstream dependencies are not Python names. They are stated directly at
the boundary that owns them and mirrored in the manifest's
`boundary_obligations` block:

- `MANIFEST_KIND_DIRECTORIES` is the authority for the complete persisted
  manifest kind-to-directory mapping. Each listed pair is stable; unknown kinds
  reject instead of inventing a directory.
- `python -m feedbax preflight-experiment-envelope` returns 0 for accepted
  input, 2 for a typed domain rejection, and 1 for infrastructure failure.
- In `feedbax.spec.graph.v5`, a component discriminator remains at
  `/graph/components/*/type`. A shape change must preserve that path, migrate it
  deterministically under a new version, or reject the old version explicitly.

These are behavior and durable-shape contracts. They are checked against the
runtime mapping and focused boundary tests, not inferred by searching for
ordinary path words or JSON field names in downstream text.
...
`external/feedbax_conformance_fixture/src/feedbax_external_conformance/policy_manifest.v2.json`.
Its row-level `public_api` blocks are the structured authority for stable imports
and commands. The additional `plugin-bootstrap.plugin_api` block owns the plugin
family, callback, support, and consumer relationships. This rendering reproduces
that plugin detail in exact order:

Policy-manifest v1 did not state row-level public APIs or structural and command
obligations. It is not migrated by guessing those commitments from prose; policy
tools reject it and require the reviewed v2 manifest.
...
Nested terminal certification remains covered by focused in-repo migration and
rejection tests rather than claiming a false external case. It is part of the
`TrainingRunManifest` lifecycle contract, not a standalone import row.
