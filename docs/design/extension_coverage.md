...
| `E21` | `feedbax/contracts/row_index.py:AuthenticatedRowIndex`, `feedbax/contracts/row_index.py:RowIndexCustodyBindings`, `feedbax/contracts/row_index.py:expand_row_selector`, `feedbax/contracts/figure_roles.py:FigureRowExpansionRequest`, `feedbax/contracts/figure_roles.py:resolve_figure_input_roles`, `feedbax/contracts/figure_roles.py:expand_figure_rows`, `feedbax/contracts/experiment_envelope.py:ExperimentEnvelopeCompilerRegistry`, `feedbax/__main__.py:preflight-experiment-envelope` | `external/feedbax_conformance_fixture/src/feedbax_external_conformance/cases.py:check_figure_role_reference_public_contract`, `tests/test_figure_role_references.py`, `tests/test_experiment_envelope_dispatch.py` | Row-set selection is a closed two-member tagged union expanded once into an explicit ordered row-id list pinned to its source index digest; empty, duplicate, and ambiguous selections fail closed with stable codes. The authenticated row index splits at the custody boundary, so compile-time identity/order/tags resolve without any post-run production record and authenticated artifact custody attaches afterwards. Figure inputs are per-row or shared role references; authored authority blocks are rejected by field and resolved digests stay in the compile lock. Row-index order alone derives the row namespace, panel placement and titles, legend ownership, colorbar placement, and assembler height, producing unchanged FigureSpec v2 semantics. One generic entrypoint dispatches an authored envelope to the registered downstream compiler claiming its schema through the injected registry, with no import-time discovery. |
