...
| `E12` | `feedbax/contracts/training.py:DeclaredTrainingProgram`, `feedbax/contracts/training.py:declare_training_program`, `feedbax/plugins/application.py:TrainingProgramCatalog`, `feedbax/training/authoring.py:compile_training_method_authoring`, `feedbax/training/executor.py:execute_training_run_spec` | `tests/test_training_authoring.py`, `tests/test_training_method_plugin_cli.py`, `tests/test_training_row_lowering.py:test_orchestration_cli_discovers_and_lowers_downstream_rows` | One declaration supplies only the runtime, authoring, row-compiler, preparation, and projection facets it needs. Application composition derives the private row-lowering and preparation registries; downstream plugins declare only `training_programs`. |
