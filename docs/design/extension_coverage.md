...
| `E16` | `feedbax/analysis/specs.py:resolve_analysis_inputs`, `feedbax/analysis/specs.py:EvaluationStateMaterializationReceipt`, `feedbax/analysis/evaluation_rows.py:project_verified_evaluation_rows`, `feedbax/analysis/evaluation_rows.py:require_exact_authored_cartesian_coverage` | `tests/test_evaluation_row_projection.py:test_projects_all_resolver_source_kinds_with_truthful_receipts`, `tests/test_evaluation_row_projection.py:test_complete_typed_source_is_bound_at_the_receipt_boundary`, `tests/test_evaluation_row_projection.py:test_one_projector_supports_two_materially_different_consumer_shapes`, `external/feedbax_conformance_fixture/src/feedbax_external_conformance/cases.py:check_typed_evaluation_row_projection` | Resolver-issued runtime receipts bind the exact state object and complete typed durable, manifest-keyed-cache, or authenticated-recompute source without overstating content authentication. Verified manifest/producer provenance then reaches one downstream-owned cross-field state/parameter/metadata/row-key projection. Stable reason codes and exact authored-Cartesian deltas are public; scientific membership and verdict policy stay downstream. |
