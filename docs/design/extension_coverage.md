...
|---|---|---|---|
| `E1` | `feedbax/component_registry/registry.py:ComponentRegistry`, `feedbax/plugins/application.py:COMPONENTS`, `feedbax/contracts/graphs/serialization.py:spec_to_graph` | `tests/test_component_registration.py:test_entry_point_component_registration_records_package_provenance`, `tests/test_component_registration.py:test_downstream_migration_pack_can_migrate_owned_component_id` | Static external components register into a fresh application bundle, migrate, materialize, and execute through explicit registry injection. Dynamic-port policy is not part of the definition. |
...
| `E14` | `feedbax/testing/suite.py:load_suite_manifest`, `feedbax/orchestration/conformance.py:CheckRegistry`, `feedbax/orchestration/conformance.py:build_default_check_registry`, `external/feedbax_conformance_fixture/src/feedbax_external_conformance/result.py:ConformanceResult` | `tests/test_run_conformance.py:test_plugin_check_discovery_and_failure_propagation`, `tests/test_external_conformance_fixture.py`, `external/feedbax_conformance_fixture/src/feedbax_external_conformance/cases.py:check_unified_plugin_bootstrap` | The versioned clean-installed fixture now proves the unified plugin bootstrap, selected public extension seams, exact current case inventory, and explicit old-version rejection. Exhaustive evidence for every lifecycle cell and the ratified compatibility window remain separately owned. |
