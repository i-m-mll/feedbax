...
| `E2` | `feedbax/plugins/bootstrap.py:discover_plugin_registrations`, `feedbax/plugins/bootstrap.py:bootstrap_application`, `feedbax/plugins/application.py:ApplicationRegistryBundle` | `tests/test_plugin_discovery_import_order.py:test_importing_plugins_does_not_eagerly_discover`, `tests/test_plugin_bootstrap.py` | One typed entry-point group discovers declarations without import-time side effects, validates protocols and dependencies, registers transactionally in deterministic order, and publishes a sealed caller-owned bundle with provenance. |
