...
1. the durable controller has no retry-admission operation and no provider-inventory
   path for detecting or handling orphan resources;
2. the retained Tier A result pins earlier Feedbax and rlrmp2 revisions and does not
   contain the required paired legacy/replacement differential evidence;
3. release-boundary Tier B has not run;
4. Feedbax publishes a SISU-specific workflow helper instead of leaving that science in
   rlrmp2; and
5. rlrmp2's exemplar supplies components and training programs through public
...
| Feedbax compiler-authority candidate | `PENDING_PARENT_INTEGRATION_REVISION` |
...
|---|---|---|---|---|
| `I01` | Graph compiler public surface: `GraphDocument`, `DocumentRoot`, `ResolvedGraph`, `ExecutableGraph`, `CompilationRecord`, `CompilationFailureRecord`, `CompilerDiagnostic`, `CompilerPhase`, `GraphSourceMap`, `GraphSourceMapEntry`, `GraphKeySchedule`, `compile_graph`; schemas `feedbax.graph_document@1`, `feedbax.resolved_graph@2`, `feedbax.graph_compilation_record@3`, `feedbax.graph_compilation_failure@1`, key schedule `feedbax.graph_key_schedule.execution_order_split.v1`. Producers are the CLI, analysis controller, Studio graph service, and worker; rlrmp2 is a consumer through its literal `GraphSpec`. | MIGRATED | Feedbax compiler. `F:feedbax/compiler/__init__.py::CompilerDiagnostic`; `F:feedbax/compiler/graph.py::class CompilationFailureRecord`; `F:tests/test_graph_compiler.py::test_compile_failure_is_a_stable_source_mapped_phase_diagnostic` | PASS: expected compilation failures carry stable code, severity, one of the seven ordered phases, revision-pinned source anchor, expected and observed conditions, and actionable context. |
...
| `I12` | Controller surface: `RunIntent`, `EffectReservation`, `ControllerEvent`, `ControllerEventStore`, `ControllerProjection`, `ReservationProjection`, `EffectObservation`, `EffectAdapter`, `DurableController`, loaders/projector; four v1 schema families. Studio produces intents and authentication; adapters produce observations; the event projector is state authority. | MIGRATED | Feedbax orchestration. `F:feedbax/orchestration/controller.py::class DurableController`; `F:tests/test_durable_controller.py::test_authenticated_ambiguous_dispatch_recovers_once_after_restart` | **FAIL**: retry and orphan inventory/handling are absent. |
...
| `I19` | Existing `ExperimentEnvelope`, compile locks, and compiled product records remain the current versioned authoring grammar and produce inputs lowered into `WorkflowPlan`. The compiler separately owns `ExperimentDocument`, `CampaignDocument`, and `ResolvedExperiment`; none aliases or reinterprets the envelope grammar. | RETAINED | Disjoint Feedbax envelope and compiler owners. `F:feedbax/compiler/experiment.py::class ExperimentDocument`; `F:feedbax/compiler/experiment.py::class CampaignDocument`; `F:feedbax/compiler/experiment.py::class ResolvedExperiment`; `F:tests/test_scientific_compiler_roots.py::test_experiment_resolves_to_one_content_pinned_identity` | PASS: the retained authoring grammar and the new exact scientific roots have separate schema identities, owners, and lifetimes. |
...
| `S15-21` | Controller retry admission and policy | MIGRATED | `F:feedbax/orchestration/controller.py::retry_admitted` | **FAIL**: the event reducer accepts the spelling, but no controller method admits a retry, applies invocation retry policy, or proves same-effect-key behavior after reconciliation. |
...
| `S15-24` | Controller orphan detection and handling | MIGRATED | `F:feedbax/orchestration/controller.py::class EffectAdapter` | **FAIL**: adapters can reconcile only a known reservation; no inventory operation can discover a provider resource lacking a live reservation/attempt, and no orphan test exists. |
...
|---|---|---|---|---|
| `S18-01` | One owner and typed identity domain for every authoritative root, IR, intent, reservation, attempt, event, artifact, and checkpoint | MIGRATED | `F:feedbax/compiler/experiment.py::class ExperimentDocument`; `F:feedbax/compiler/experiment.py::class CampaignDocument`; `F:feedbax/compiler/experiment.py::class ResolvedExperiment`; `F:feedbax/orchestration/controller.py::class ControllerEventStore`; `F:feedbax/contracts/publication.py::class CheckpointSet`; `F:tests/test_scientific_compiler_roots.py::test_experiment_resolves_to_one_content_pinned_identity` | PASS: the compiler owns the three formerly missing typed roots; the concrete append-only controller event store remains the event-log authority under its explicit event schema rather than a second alias type. |
...
| `S18-08` | Every external effect reserved before dispatch, ambiguity reconciled, cloud attempts authorized by exact unexpired reservation | MIGRATED | `F:feedbax/orchestration/controller.py::external_effect_dispatched`; `F:tests/test_durable_controller.py::test_authenticated_ambiguous_dispatch_recovers_once_after_restart` | **FAIL**: the admitted controller surface lacks retry and orphan behavior, so the universal effect/reconciliation claim is not proven. |
...
1. repair retry admission and orphan inventory/handling, or explicitly revise those
   obligations through an owner decision;
