...
|---|---|---|---|---|
| `S18-01` | One owner and typed identity domain for every authoritative root, IR, intent, reservation, attempt, event, artifact, and checkpoint | MIGRATED | `F:feedbax/compiler/graph.py::class GraphDocument`; `F:feedbax/orchestration/controller.py::class RunIntent`; `F:feedbax/contracts/publication.py::class CheckpointSet` | **FAIL**: `ExperimentDocument`, `CampaignDocument`, and `ResolvedExperiment` are absent; `EventLog` exists only as the concrete `ControllerEventStore` JSONL authority rather than the complete named root census. |
