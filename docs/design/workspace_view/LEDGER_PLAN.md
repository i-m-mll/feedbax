...
|---|---|---|---|
| C1 | Representation contract schemas + registry plumbing | `RepresentationSpec` (archetypes, anchors, bindings, style channels, frame providers) as versioned Pydantic contracts; optional field on `ComponentMeta`/`ComponentDefinition`; catalog serving; TS mirrors + `parseContract` wiring; unrepresented-placeholder fallback. (§§2–3) | — (anchor child; refines `946c859`) |
