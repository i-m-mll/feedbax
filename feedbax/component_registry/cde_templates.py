...
def register_cde_templates(registry: _Registry) -> None:
    ...
    # ------------------------------------------------------------------ #
    def _node(
        type_: str, input_ports: list[str], output_ports: list[str], **params
    ) -> ComponentSpec:
        if type_ == "Input":
            input_ports = ["input"]
        ...
        return ComponentSpec(
            type=type_, params=params, input_ports=input_ports, output_ports=output_ports
        )
