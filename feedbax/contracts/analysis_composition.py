...
class AnalysisRunDeltaSpec(StrictModel):
    ...
    def _validate_delta_spec(self) -> "AnalysisRunDeltaSpec":
        if self.schema_id != ANALYSIS_RUN_DELTA_SPEC_SCHEMA_ID:
            raise ValueError(
                f"unsupported AnalysisRunDeltaSpec schema_id {self.schema_id!r}"
            )
...
def flatten_analysis_run_delta(
    ...
    *,
    ...
    chain: list[tuple[str, AnalysisRunDeltaSpec]] = []
    seen: set[str] = set()
    current = spec
    while True:
        digest = analysis_run_delta_envelope_hash(current)
        if digest in seen:
            raise ValueError("analysis run delta parent composition cycle detected")
        seen.add(digest)
        ...
        if parent_schema_id == ANALYSIS_RUN_DELTA_SPEC_SCHEMA_ID:
            current = AnalysisRunDeltaSpec.model_validate(parent_payload)
            continue
        if parent_schema_id != ANALYSIS_RUN_SPEC_SCHEMA_ID:
            raise ValueError(
                "analysis run delta parent must declare schema_id "
                f"{ANALYSIS_RUN_SPEC_SCHEMA_ID!r} or "
                f"{ANALYSIS_RUN_DELTA_SPEC_SCHEMA_ID!r}, got {parent_schema_id!r}"
            )
        payload = default_spec_registry.migrate("AnalysisRunSpec", parent_payload).payload
        break

    attribution: dict[str, str] = {}
    written: set[str] = set()
    layers: list[AnalysisCompositionLayer] = []
