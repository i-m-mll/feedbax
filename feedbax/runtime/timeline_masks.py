...
The lowering itself is pure mask arithmetic over duck-typed task-spec mappings,
so it lives with the graph runtime that consumes it rather than with concrete
task definitions. ``feedbax.tasks.timeline_masks`` re-exports these names for
task-facing callers.
"""
